...
CREATE TABLE IF NOT EXISTS CUSTOMERS (
    ...
    LOYALTY_SCORE       NUMBER(6,2)    DEFAULT 0,   -- computed from points balance
    -- SCD2 fields
...
USING (
    SELECT DISTINCT
        ...
        COALESCE(lp.TOTAL_POINTS / 100.0, 0)                      AS LOYALTY_SCORE,
    ...
    LEFT JOIN RAW.LOYALTY_POINTS lp
        ON lp.PAYLOAD:customer_id::VARCHAR = c.PAYLOAD:customer_id::VARCHAR
    WHERE c.PAYLOAD:customer_id IS NOT NULL
) src ON (tgt.CUSTOMER_ID = src.CUSTOMER_ID AND tgt.IS_CURRENT = TRUE AND src.RN = 1)
WHEN MATCHED AND (
    tgt.EMAIL != src.EMAIL OR tgt.PHONE != src.PHONE OR
    tgt.CUSTOMER_TIER != src.CUSTOMER_TIER OR tgt.IS_ACTIVE != src.IS_ACTIVE OR
...
WHEN NOT MATCHED AND src.RN = 1 THEN INSERT (
    ...
    DATE_OF_BIRTH, GENDER, CUSTOMER_TIER, REGISTRATION_DATE, IS_ACTIVE,
    LOYALTY_SCORE, SRC_LOAD_TIMESTAMP
) VALUES (
    src.CUSTOMER_ID, src.FIRST_NAME, src.LAST_NAME, src.FULL_NAME, src.EMAIL, src.PHONE,
    src.DATE_OF_BIRTH, src.GENDER, src.CUSTOMER_TIER, src.REGISTRATION_DATE, src.IS_ACTIVE,
    src.LOYALTY_SCORE, src.SRC_LOAD_TIMESTAMP
);
